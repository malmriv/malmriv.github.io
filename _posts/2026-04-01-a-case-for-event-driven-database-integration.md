...
The trouble starts when the data load grows, or when it becomes uneven. Imagine a burger stand that starts small and eventually becomes a chain. Orders that used to trickle in throughout the day now arrive in bursts: lunchtime, dinnertime, weekends. The polling job that worked fine at fifty orders a day now faces ten thousand, all piled up at the same hour and right when the database is already under its heaviest transactional load. Delays happen, relevant servers downstream might be down for se cond. And if a run fails halfway through, you are left choosing between reprocessing the whole batch (risking duplicates in the target system) or skipping what already ran (leaving gaps). And in the end, human intervention becomes necessary. The root cause is not the volume itself, but the philosophy behind this architecture: polling turns a continuous stream of events into a batch, and batches, by definition, accumulate.
